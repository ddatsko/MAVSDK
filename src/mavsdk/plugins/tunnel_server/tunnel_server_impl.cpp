...
bool TunnelServerImpl::send_tunnel_mavlink_message(
...
TunnelServerImpl::TunnelServerImpl(std::shared_ptr<ServerComponent> server_component) :
...
TunnelServer::Result TunnelServerImpl::send_tunnel_message(
    uint32_t target_system,
    uint32_t target_component,
    uint32_t payload_type,
    uint32_t payload_length,
    std::vector<std::byte> payload)
{
    ...
    if (!send_tunnel_mavlink_message(
...
TunnelServerImpl::subscribe_tunnel_message(const TunnelServer::TunnelMessageCallback& callback)
{
    std::lock_guard<std::mutex> lock(_tunnel_message_mutex);
