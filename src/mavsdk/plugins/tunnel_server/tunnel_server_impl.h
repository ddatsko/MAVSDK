...
public:
    ...
    TunnelServer::Result send_tunnel_message(uint32_t target_system, uint32_t target_component, uint32_t payload_type, uint32_t payload_length, std::vector<std::byte> payload);



        
    TunnelServer::TunnelMessageHandle subscribe_tunnel_message(const TunnelServer::TunnelMessageCallback& callback);
